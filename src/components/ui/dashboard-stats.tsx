...
import { Users, Trophy, DollarSign, Ticket, TrendingUp, Calendar, Star, Award } from "lucide-react";
...
const mockData = {
  ...
  recentSales: [
    ...
    { id: 3, buyer: "Carlos Ruiz", tickets: 3, amount: 150, date: "2024-01-14" },
    { id: 4, buyer: "Lucia Torres", tickets: 12, amount: 600, date: "2024-01-14" },
    { id: 5, buyer: "Roberto Díaz", tickets: 2, amount: 100, date: "2024-01-13" },
  ],
  dailyStats: [
...
export const DashboardStats = () => {
  const soldPercentage = Math.round((mockData.ticketsSold / mockData.totalTickets) * 100);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="border-2 border-primary/20 bg-gradient-to-br from-primary/5 to-primary/10 hover:shadow-lg transition-all">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-secondary">Total Recaudado</CardTitle>
            <DollarSign className="h-4 w-4 text-primary" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-secondary">${mockData.totalSales.toLocaleString()}</div>
        ...
        <Card className="border-2 border-primary/20 bg-gradient-to-br from-secondary/5 to-secondary/10 hover:shadow-lg transition-all">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-secondary">Tickets Vendidos</CardTitle>
            <Ticket className="h-4 w-4 text-secondary" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-secondary">{mockData.ticketsSold}</div>
        ...
        <Card className="border-2 border-primary/20 bg-gradient-to-br from-accent/5 to-accent/10 hover:shadow-lg transition-all">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-secondary">Tickets Restantes</CardTitle>
            <Trophy className="h-4 w-4 text-accent" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-secondary">{remainingTickets}</div>
            <p className="text-xs text-accent">
        ...
        <Card className="border-2 border-primary/20 bg-gradient-to-br from-primary/10 to-secondary/5 hover:shadow-lg transition-all">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-secondary">Mayor Comprador</CardTitle>
            <Users className="h-4 w-4 text-primary" />
          </CardHeader>
          <CardContent>
            <div className="text-lg font-bold text-secondary">{mockData.topBuyer.name}</div>
            <p className="text-xs text-accent">
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-secondary">Vendidos: {mockData.ticketsSold}</span>
              <span className="text-sm text-accent">Restantes: {remainingTickets}</span>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-accent">${mockData.totalSales.toLocaleString()} recaudado</span>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {mockData.recentSales.map((sale) => (
                <div key={sale.id} className="flex items-center justify-between p-3 bg-primary/5 rounded-lg">
                  <div>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm text-accent">{sale.tickets} tickets</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              {mockData.dailyStats.map((day, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-secondary/5 rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-accent">{day.tickets} tickets vendidos</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-secondary">${day.sales.toLocaleString()}</p>
                    <Badge 
                      variant="outline" 
                      className={index === 0 ? "border-primary text-primary" : "border-accent/30 text-accent"}
                    >
