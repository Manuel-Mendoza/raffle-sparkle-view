...
export function HeroSection({
  ...
  // Use individual winners if available, otherwise fall back to legacy winners
  const hasIndividualWinners = individualWinners && (
    individualWinners.first || individualWinners.second || individualWinners.third
  );
  const hasLegacyWinners = winners && winners.length > 0;
  const hasWinners = hasIndividualWinners || hasLegacyWinners;
          ...
          <>
            ...
            {hasIndividualWinners && onRefetchWinners && (
              <div className="mb-6">
                ...
                >
                  <RefreshCw className={`w-4 h-4 mr-2 ${winnersLoading ? 'animate-spin' : ''}`} />
            ...
            {hasIndividualWinners && (
              <div className="space-y-4 mb-8">
                ...
                >
                  🥇 1er Lugar: {
                    individualWinners.first 
                      ? `${individualWinners.first.customerName} - Ticket ${individualWinners.first.ticketNumber.padStart(4, "0")}`
                      : "Por anunciar..."
                  }
                </Badge>
                ...
                {/* Second Place */}
                <Badge
                  className={`bg-gray-400/20 text-gray-300 border-gray-400/30 backdrop-blur-sm text-lg px-6 py-3 font-semibold block ${
                    winnersLoading ? 'opacity-50' : ''
                  }`}
                >
                  🥈 2do Lugar: {
                    individualWinners.second 
                      ? `${individualWinners.second.customerName} - Ticket ${individualWinners.second.ticketNumber.padStart(4, "0")}`
                      : "Por anunciar..."
                  }
                </Badge>
                ...
                {/* Third Place */}
                <Badge
                  className={`bg-amber-600/20 text-amber-300 border-amber-500/30 backdrop-blur-sm text-lg px-6 py-3 font-semibold block ${
                    winnersLoading ? 'opacity-50' : ''
                  }`}
                >
                  🥉 3er Lugar: {
                    individualWinners.third 
                      ? `${individualWinners.third.customerName} - Ticket ${individualWinners.third.ticketNumber.padStart(4, "0")}`
                      : "Por anunciar..."
                  }
                </Badge>
              </div>
            ...
            {/* Legacy Winners Display (fallback) */}
            {!hasIndividualWinners && hasLegacyWinners && (
              <div className="space-y-4 mb-8">
